...
export const availableTemplates: Template[] = [
    ...
    },
    {
        ...
        whatDoInfo: [
            { label: 'Checkout', description: 'Download your repository on the runner', icon: 'IoDownloadOutline' },
            { label: 'Set destination branch', description: 'Select the branch that will trigger releases (e.g., main).', icon: 'IoGitBranchOutline' },
            { label: 'Define Node version', description: 'Pin the Node.js version for reproducible builds', icon: 'IoLogoNodejs' },
            { label: 'Install dependencies', description: 'Install project dependencies (npm/yarn/pnpm)', icon: 'IoConstructOutline' },
            { label: 'Build', description: 'Create production artifacts if your project needs them', icon: 'IoBuildOutline' },
            {
                label: 'Launch Semantic Release',
                description: 'Analyze commits, compute next version, create tag and GitHub Release.',
                icon: 'IoRocketOutline',
            },
        ],
        requirements: [
            {
                description: 'Commit messages follow Conventional Commits or you provide a configuration that maps commit types to release types.',
                icon: 'IoDocumentTextOutline',
            },
            {
                description: 'A semantic-release configuration (package.json "release" field or a .releaserc file) with the plugins you need (e.g., github, changelog, npm, git).',
                icon: 'IoSettingsOutline',
            },
            {
                description: 'A GitHub token stored as a repository secret (e.g., GH_TOKEN).',
                icon: 'IoKeyOutline',
            },
            {
                description: 'Repository Actions permissions allow writing releases and tags (contents: write) or use a PAT with repo scope.',
                // eslint-disable-next-line no-secrets/no-secrets
                icon: 'IoShieldCheckmarkOutline',
            },
            { description: 'Runs on ubuntu-latest. Supports Node 18, 20, and 22.', icon: 'IoServerOutline' },
        ],
        faq: [
            {
                label: 'How is the next version decided?',
                description: 'Based on Conventional Commits: fix → patch, feat → minor, and commits marked as BREAKING CHANGE → major (plus configured rules).',
                icon: 'IoGitCommitOutline',
            },
            {
                label: 'Do I need a config file?',
                description: 'Not strictly, but recommended. Add a .releaserc (or "release" in package.json) to control plugins like changelog, github, npm, and git.',
                icon: 'IoSettingsOutline',
            },
            {
                label: 'Can it publish to npm too?',
                description: 'Yes. Add @semantic-release/npm in your config and provide an NPM_TOKEN secret. The workflow here focuses on GitHub Releases by default.',
                icon: 'IoCubeOutline',
            },
            {
                label: 'Why use GH_TOKEN if GITHUB_TOKEN exists?',
                description:
                    'You can use GITHUB_TOKEN, but some setups prefer a PAT (GH_TOKEN) with repo scope for consistent permissions. Map it to env GITHUB_TOKEN in the step.',
                icon: 'IoKeyOutline',
            },
            {
                label: 'Will it run on every push?',
                description: 'Only on pushes to the configured branch (e.g., main). Adjust on.push.branches to change when releases are attempted.',
                icon: 'IoGitBranchOutline',
            },
            {
                label: 'What does the release contain?',
                description: 'A Git tag and a GitHub Release with autogenerated notes. Extra plugins can update CHANGELOG.md and commit the file back to the repo.',
